...
const fetchData = (): Promise<DataType[]> => {
  return BaseApi.get(BASE_URL).then((res) => res.data.content)
}

export default function TableItemRoadMap({ }: Props) {

   const {data} = useQuery({queryKey: ['query-tutorials'], queryFn: fetchData});
  ...
  let dataRender: DataType[] = data || [] as DataType[];
